...
UnityLight MainLight (half3 normalWorld)
{
	UnityLight l;
	#ifdef LIGHTMAP_OFF
		l.color = _LightColor0.rgb + _LightColor0.rgb;
	...
	UnityLight light)
{
	...
	d.atten = atten;
	d.ambientOrLightmapUV = i_ambientOrLightmapUV;
...
half4 fragForwardBase (VertexOutputForwardBase i) : SV_Target
{
	...
	half4 colorFresnel = UNITY_BRDF_PBS (s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, -s.eyeVec, gi.light, gi.indirect);
	colorFresnel.rgb += UNITY_BRDF_GI (s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, -s.eyeVec, gi);
...
half4 fragForwardAdd (VertexOutputForwardAdd i) : SV_Target
{
	...
	half4 colorFresnel = UNITY_BRDF_PBS (s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, -s.eyeVec, light, noIndirect);
	half3 color = colorFresnel.rgb;
	half fresnel = colorFresnel.a;
