...
namespace psai.net
{
    ...
    internal interface LoggerOutput
    {
    ...
    internal class LoggerOutputWindows : LoggerOutput
    {
        ...
        {
            string pathToAppDataDir = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
    ...
    internal class Logger
    {
        ...
        private LoggerOutput m_loggerOutput;
        ...
        internal LoggerOutput LoggerOutput
        {
            get
            {

                if (m_loggerOutput == null)
                {
                    #if PSAI_WIN
                        m_loggerOutput = new LoggerOutputWindows();                    
                    #else
                        m_loggerOutput = new LoggerOutputUnity();
                    #endif

                }
                return m_loggerOutput;
            }

            set
            {
                m_loggerOutput = value;
            }
        }
        ...
        internal void Log(string argText, LogLevel logLevel)
        {
            if (logLevel <= LogLevel)
            {
                ...
                LoggerOutput.WriteLog(sb.ToString(), logLevel);
