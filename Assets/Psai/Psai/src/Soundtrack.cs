...
namespace psai.net
{
    public class Soundtrack
    {
        ...
        public SegmentInfo getSnippetInfo(int snippetId)
        {
            SegmentInfo snippetInfo = new SegmentInfo();

            Segment snippet = GetSegmentById(snippetId);

            if (snippet != null)
            {
                snippetInfo.id = snippet.Id;
                snippetInfo.intensity = snippet.Intensity;
                snippetInfo.segmentSuitabilitiesBitfield = snippet.SnippetTypeBitfield;
                snippetInfo.themeId = snippet.ThemeId;
                snippetInfo.playcount = snippet.Playcount;
                snippetInfo.name = snippet.Name;
                snippetInfo.fullLengthInMilliseconds = snippet.audioData.GetFullLengthInMilliseconds();
                snippetInfo.preBeatLengthInMilliseconds = snippet.audioData.GetPreBeatZoneInMilliseconds();
                snippetInfo.postBeatLengthInMilliseconds = snippet.audioData.GetPostBeatZoneInMilliseconds();
            }
