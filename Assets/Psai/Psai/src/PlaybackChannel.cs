...
namespace psai.net
{
    ...
    internal class PlaybackChannel
    {
        ...
        internal void LoadSnippet(Segment snippet)
        {
            Segment = snippet;
            ...
            {
                m_audioPlaybackLayerChannel.LoadSnippet(snippet);
            }
        }


        internal bool CheckIfSnippetHadEnoughTimeToLoad()
        {
        ...
        internal void ReleaseSnippet()
        {
            Segment = null;

            if (m_audioPlaybackLayerChannel != null)
            {
                m_audioPlaybackLayerChannel.ReleaseSnippet();
            }
        }
        ...
        internal void ScheduleSnippetPlayback(Segment snippet, int delayInMilliseconds)
        {            
            #if !(PSAI_NOLOG)
            {
                if (LogLevel.debug <= Logger.Instance.LogLevel)
                {
	                StringBuilder sb = new StringBuilder();
	                sb.Append("ScheduleSnippetPlayback() snippet=");
            ...
            if (m_audioPlaybackLayerChannel != null)
            {
                m_audioPlaybackLayerChannel.ScheduleSnippetPlayback(snippet, delayInMilliseconds);
            }
            else
            {
                #if !(PSAI_NOLOG)
