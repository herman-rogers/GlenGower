...
namespace psai.net
{
    ...
    {
        private static readonly string PSAI_VERSION = ".NET 1.5.13";
        ...
        PsaiTimer m_timerSnippetEndApproaching = new PsaiTimer();
        PsaiTimer m_timerSnippetEndReached = new PsaiTimer();
        ...
        {
            #if PSAI_WIN
                m_platformLayer = new PlatformLayerWindows(this);
            #else
            ...
            m_fullVersionString = PSAI_VERSION;

            s_instance = this;      // TODO: wenn ich mehrere Instanzen von PsaiCore zulasse, funktioniert das Singleton dann hier noch?
        }
        ...
        // TODO: move PsaiProject to common namespace
        /*
        internal PsaiResult LoadSoundtrackFromProjectFile(string pathToProjectFile)
        {
            ...
            using (Stream stream = m_platformLayer.GetStreamOnPsaiCoreBinary(m_psaiCoreBinaryFilepath))
            {
                ...
                if (project != null)
                {
                    ...
                    m_soundtrack = project.BuildPsaiDotNetSoundtrackFromProject();
        ...
        */
        ...
        internal PsaiResult LoadSoundtrack(string pathToPcbFile)
        {
                ...
                if (LogLevel.info <= Logger.Instance.LogLevel)
                {
                    Logger.Instance.Log(PSAI_VERSION, LogLevel.info);
                }
            #endif
        ...
        internal bool setLogLevel(LogLevel newLogLevel)
	    {
		    if (newLogLevel >= LogLevel.off && newLogLevel <= LogLevel.debug)
		    {
			    #if !(PSAI_NOLOG)
			    {
                    if (LogLevel.info <= Logger.Instance.LogLevel)
                    {
                    	Logger.Instance.Log("setLogLevel() " + newLogLevel, LogLevel.info);
                    }
			    }
			    #endif
	    ...
	    * This method only makes checks for channel index boundaries and ignores the playback states of the channels.
	    */
        internal int getNextVoiceNumber(bool forHighlight)
        {
            ...
            if (!forHighlight)
            {
                nextVoiceNumber = m_currentVoiceNumber + 1;
        ...
        internal PsaiResult setPaused(bool setPaused)
        {
            ...
            if ((setPaused && !m_paused) || (!setPaused && m_paused))
            {
                ...
                m_timerSnippetEndApproaching.SetPaused(setPaused);
                m_timerSnippetEndReached.SetPaused(setPaused);
        ...
        internal PsaiResult Update()
	    {
            if (!m_paused)
            {
                ...
                if (m_timerSnippetEndApproaching.ThresholdHasBeenReached())
                {
                    m_timerSnippetEndApproaching.Stop();
                    SegmentEndApproachingHandler();
                }


                if (m_timerSnippetEndReached.ThresholdHasBeenReached())
                {
                    m_timerSnippetEndReached.Stop();
	    ...
	    {				
		    #if !(PSAI_NOLOG)
                {
                    if (LogLevel.info <= Logger.Instance.LogLevel)
                    {
	                    ...
	                    sb.Append(Segment.getStringOfSegmentTypes(m_currentSnippetTypeRequested));
	                    Logger.Instance.Log(sb.ToString(), LogLevel.info);
                    }
                }
		    #endif
        ...
        internal PsaiResult TriggerMusicTheme(Theme argTheme, float argIntensity, int argMusicDuration)
        {
		    ...
		    if (effectiveSegment.ThemeId == argTheme.id)
		    {
			    #if !(PSAI_NOLOG)
			    {
                    if (LogLevel.debug <= Logger.Instance.LogLevel)
                    {
	                    ...
	                    sb.Append("theme ");
	                    sb.Append(argTheme.Name);
	                    sb.Append(" is already playing and the snippet end approaching timer is pending. Updating intensity to ");
	                    sb.Append(argIntensity);
	                    Logger.Instance.Log(sb.ToString(), LogLevel.debug);
                    }
			    }
		    ...
		    case ThemeInterruptionBehavior.immediately:
			    {
                    if (argTheme.themeType == ThemeType.shockEvent)
                    {
        ...
        internal void PlaySegmentLayeredAndImmediately(Segment segment)
        {
            ...
            m_playbackChannels[m_hilightVoiceIndex].ReleaseSnippet();

            m_playbackChannels[m_hilightVoiceIndex].FadeOutVolume = 1.0f;
            m_playbackChannels[m_hilightVoiceIndex].ScheduleSnippetPlayback(segment, s_audioLayerMaximumLatencyForBufferingSounds + s_audioLayerMaximumLatencyForPlayingbackPrebufferedSounds);
        }
	    ...
	    */
	    PsaiResult PlaySegment(Segment targetSnippet, bool immediately)
	    {
		    ...
		    //////////////////////////////////////////////////////////////////////////
		    // at this point it's sure that the requested snippet will be played back, either immediately or after the end of the current snippet
            ...
            */
		    ...
		    // ausgabe vorbereiten
		    psaiResult =  LoadSnippet(targetSnippet, m_targetVoice);

		    PsaiErrorCheck(psaiResult, "loadSnippet()");		
			    ...
			    else 
			    {
				    #if !(PSAI_NOLOG)
				    {
                        if (LogLevel.debug <= Logger.Instance.LogLevel)
                        {
	                        ...
	                        sb.Append(s_audioLayerMaximumLatencyForPlayingBackUnbufferedSounds);
	                        sb.Append("  m_currentSnippetPlaying->FullLengthInMs=");
        ...
        PsaiResult LoadSnippet(Segment snippet, int channelIndex)
        {
            ...
            {
                m_playbackChannels[channelIndex].LoadSnippet(snippet);
                return PsaiResult.OK;
            }
        }

        
        /* This method will play the target Snippet with minimum delay, and set the m_targetSnipet to m_currentSnippet, and m_targetVoice to m_currentVoice.
         */        
	    void PlayTargetSegmentImmediately()
	    {
            #if !(PSAI_NOLOG)
            {
                if (LogLevel.debug <= Logger.Instance.LogLevel)
                {
	                ...
	                sb.Append(Segment.getStringOfSegmentTypes(m_targetSegmentSuitabilitiesRequested));
            ...
            if (m_playbackChannels[m_targetVoice].CheckIfSnippetHadEnoughTimeToLoad())
            {
            ...
            m_playbackChannels[m_targetVoice].ScheduleSnippetPlayback(m_targetSegment, snippetPlaybackDelayMillis);            
            ...
            m_timerSnippetEndApproaching.SetTimer(millisUntilNextCalculateCall, s_updateIntervalMillis);
            ...
            m_timerSnippetEndReached.SetTimer(millisUntilUpcomingSnippetEnd, 0);
            ...
            #if !(PSAI_NOLOG)
            {
                ...
                {
	                StringBuilder sb = new StringBuilder();
	                sb.Append("m_timer_SnippetEndApproaching should fire in ");
	                sb.Append(millisUntilNextCalculateCall);
	                sb.Append(" ms");
	                Logger.Instance.Log(sb.ToString(), LogLevel.debug);
	                ...
	                sb2.Append("m_timer_SnippetEndReached should fire in ");
	    ...
	    */
	    PsaiResult PlayThemeNowOrAtEndOfCurrentSegment(int themeId, float intensity, int musicDuration, bool immediately, bool holdIntensity)
	    {
            ...
            // Use Start Segments only when playing out of silence.
		    ...
		    // ////////////////////////////////////////////////
		    // at this point, the target snippet has to be set! (internal error otherwise)
		    // ////////////////////////////////////////////////
				    ...
				    else		// if the time offset is zero, fade out this voice
				    {
					    ...
					    else
					    {
                            ...
                            if (m_playbackChannels[voiceNumber].IsPlaying())
						    {
                                ...
                                m_playbackChannels[voiceNumber].ReleaseSnippet();
        ...
        void SegmentEndApproachingHandler()
	    {
		    ...
		    switch (m_psaiStateIntended)
		    {
			    case PsaiState.silence:
				    {
					    if ( m_currentSegmentPlaying == null || (m_currentSegmentPlaying.SnippetTypeBitfield & (int)SegmentSuitability.end) > 0 )
					    {
						    EnterSilenceMode();
					    }
					    else
			    ...
			    case PsaiState.rest:
				    {
                        if (m_currentSegmentPlaying == null || (m_currentSegmentPlaying.SnippetTypeBitfield & (int)SegmentSuitability.end) > 0)
                        {
                            switch (m_effectiveTheme.themeType)
                            {
                                case ThemeType.basicMood:
                                    GoToRest(m_effectiveTheme.id, m_effectiveTheme.id);
                                    break;

                                default:
                                    GoToRest(GetLastBasicMoodId(), m_effectiveTheme.id);
                                    break;
                            }
                        }
                        else
	    ...
	    */
	    void GoToRest(int themeIdToWakeUpWith, int themeIdToUseForRestingTimeCalculation)
	    {
		    //boost::recursive_mutex::scoped_lock block(m_pnxLogicMutex);
		    ...
		    #if !(PSAI_NOLOG)
            {
                ...
                {
	                StringBuilder sb = new StringBuilder();
	                sb.Append("--- Entering rest mode. Resting time is based on Theme ");
	                sb.Append(themeIdToUseForRestingTimeCalculation);
	                sb.Append(".  Will wake up with Theme ");
		    ...
		    if (m_effectiveTheme != null)
		    {
			    int millisTimerRest = GetRandomInt(themeRest.restSecondsMin, themeRest.restSecondsMax) * 1000; 
		    ...
		    if (snippetList.Count == 0)
		    {
			    ...
			    #if !(PSAI_NOLOG)
			    {
                    if (LogLevel.errors <= Logger.Instance.LogLevel)
                    {
	                    ...
	                    sb.Append(Segment.getStringOfSegmentTypes(allowedSegmentSuitabilities));
			    ...
			    case PsaiPlayMode.regular:
				    {
					    ...
					    {
						    GoToRest(tqe.themeId, tqe.themeId);
					    }
				    }
				    break;
