...
public class PsaiEditorWindow : EditorWindow
{
    ...
    void BuildFileMenu()
    {
        ...
        _fileMenu.AddItem(new GUIContent("New psai Project"), false, FileMenuCallback, "new");
        _fileMenu.AddItem(new GUIContent("Open psai Project"), false, FileMenuCallback, "open");
        ...
        if (EditorModel.Instance.Project != null)
        {
            _fileMenu.AddItem(new GUIContent("Import Theme(s) from other .psai Project"), false, FileMenuCallback, "import");
        }
        else
        {
            _fileMenu.AddDisabledItem(new GUIContent("Import Theme(s) from other .psai Project"));
        }
    ...
    public void OnNewProjectClicked()
    {
        ...
        {
            string path = EditorUtility.SaveFilePanel("New psai Project", Application.dataPath + "/Resources/", "", "psai");
            if (path.Length != 0)
            {
    ...
    void FileMenuCallback (object obj) 
    {
        ...
        {
            string path = EditorUtility.SaveFilePanel("Save psai Project", _pathToPsaiProject, "", "psai");
            if (path.Length != 0)
            {
                _pathToPsaiProject = path;
                EditorModel.Instance.SaveProjectData(path);
            }  
        }
    ...
    {       
        ...
        string path = EditorUtility.OpenFilePanel("select psai Project", _pathToPsaiProject, "psai");
        if (path.Length != 0)
        {
    ...
    void OnOpenProjectClicked()
    {
        ...
        if (CheckForUnsavedChangesAndDisplayMessageBoxToDiscard())
        {
            ...
            string path = EditorUtility.OpenFilePanel("Open psai Project", initialDirectory, "psai");
            if (path.Length != 0)
            {
        ...
        else
        {
            ...
            else
            {
                foreach (Theme theme in EditorModel.Instance.Project.Themes)
                {
                    ...
                    #if (UNITY_3_0 || UNITY_3_0_0 || UNITY_3_1 || UNITY_3_2 ||UNITY_3_3 || UNTIY_3_4 || UNITY_3_5 || UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
                        Rect foldRect = GUILayoutUtility.GetRect(10, 10);
                    #else
    ...
    private void OnBuildSoundtrackClicked()
    {
        ...
        {
            defaultPath = _pathToBinarySoundtrackFile;
        }
        ...
        _pathToBinarySoundtrackFile =  EditorUtility.SaveFilePanel("Please choose a name for the binary soundtrack file",
                                defaultPath,
								"soundtrack",
								"bytes"
								);
        ...
        if (_pathToBinarySoundtrackFile.Length > 0)
        {
            ...
            {
                EditorModel.Instance.BuildPsaiCoreBinaryFile(_pathToBinarySoundtrackFile, "OGG", null, 0, 0);
    ...
    bool CheckForUnsavedChangesAndDisplayMessageBoxToDiscard()
    {
        if (EditorModel.Instance.ProjectDataChangedSinceLastSave)
        {
            string text = "Your current psai Project contains unsaved changes. Do you wish to continue and discard your changes?";
    ...
    void OnDisable()
    {
        if (EditorModel.Instance.ProjectDataChangedSinceLastSave)
        {           
            if (EditorUtility.DisplayDialog("Save psai Project?", "Continuing will discard the changes you have made to your psai Project. Do you wish to save or discard your changes?", "Save", "Discard"))
            {
                EditorModel.Instance.SaveProjectData(_pathToPsaiProject);
